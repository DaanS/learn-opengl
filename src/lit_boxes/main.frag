...
vec3 calc_dir_light(dir_light_type light, vec3 normal, vec3 view_dir) {
    ...
    vec3 diff_color = diff_strength * light.diffuse * vec3(texture(material.diffuse, tex_coords));

    vec3 reflect_dir = reflect(-light_dir, normal);
    float spec_strength = pow(max(dot(view_dir, reflect_dir), 0.0), material.shininess);
    vec3 spec_color = spec_strength * light.specular * vec3(texture(material.specular, tex_coords));
...
vec3 calc_point_light(point_light_type light, vec3 normal, vec3 view_dir, vec3 frag_pos) {
    ...
    vec3 reflect_dir = reflect(-light_dir, normal);
    vec4 spec = texture(material.specular, tex_coords);
